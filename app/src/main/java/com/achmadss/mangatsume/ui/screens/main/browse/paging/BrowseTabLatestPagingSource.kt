...
class BrowseTabLatestPagingSource(
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, MangaModel> {
        return try {
            ...
            moreLatestLoading.update { false }
        }
    }
    ...
    private suspend fun fetchData(nextPageNumber: Int): List<MangaModel> {
        ...
        coroutineScope {
            getHotUseCase.invoke(
                GetHotUseCase.GetHotUseCaseParam(MangaTrendingType.Latest, nextPageNumber)
            ).collect { result ->
                when (result) {
